-- ============================================================
-- MIGRASI v9 — harga khusus agen (agent_prices)
-- Jalankan SEKALI di Supabase Dashboard → SQL Editor. Aman diulang.
-- ============================================================

-- ── 1. tabel harga khusus agen ──────────────────────────────────────
-- Setiap agen bisa punya harga jual khusus per produk.
-- Bila kode agen valid & aktif di pesanan, harga satuan diambil dari
-- agent_prices; bila tidak ada baris untuk (agen, produk) → pakai harga
-- normal/grosir seperti sebelumnya.
...
-- ── 1b. mode komisi agen ─────────────────────────────────────────────
-- 'percent' (bawaan) = komisi persen seperti sebelumnya.
-- 'price'            = keuntungan agen dari selisih harga khusus di atas.
-- Murni penanda untuk tampilan admin: perhitungan komisi & harga tetap
-- dilakukan create_order di bawah (harga khusus dipakai begitu ada barisnya).
alter table agents add column if not exists commission_mode text not null
  default 'percent';
do $$ begin
  alter table agents drop constraint if exists agents_commission_mode_check;
  alter table agents add constraint agents_commission_mode_check
    check (commission_mode in ('percent','price'));
end $$;

-- ── 2. update fungsi create_order: pakai harga agen bila ada ─────────
-- Perubahan: setelah kode agen divalidasi & aktif (bukan pembelian
-- sendiri), harga satuan tiap produk di v_lines ditimpa dengan
-- agent_prices bila ada. v_subtotal dihitung ulang.
...
begin
  ...
  if v_code is not null then
    ...
    if not found then
      v_code := null;                 -- kode tak dikenal → tanpa atribusi
    else
  ...
  if v_code is not null then
    ...
    if not coalesce(v_cs.aktif, false) then
      ...
      if v_cs.min_order_amount > 0 and v_basis < v_cs.min_order_amount then
